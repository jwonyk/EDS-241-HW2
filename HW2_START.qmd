...
*Response:* `log_median_inc` and `p_uni_degree` look *balanced* while `log_home_val_07` and `log_pop_denc` look *imbalanced*.

**2B.Q2** Describe in your own words why these covariates might be expected to confound the treatment estimate:

*Response (2-4 sentences):* The main threat to causal inference using fixed effects estimators is time-varying confounders—variables that change differentially in the treated and control groups. Thus, for the average treatment effect (ATE) to be interpreted causally, the parallel trends assumption must hold. To provide evidence to support the parallel trends assumption, the mean level of the outcome variable by year is plotted below, showing parallel trends pre-treatment.
...
*Response:* Prior to matching, standard mean difference of `log_median_inc` was the smallest and `log_pop_denc` was the largest. Same as prior to matching, `log_median_inc` remain to be smallest standard mean difference as well as `log_pop_denc`.
...
Yes, there is causal estimate using this method because the p-value is less than 0.05 which there is the significance and as the matched data best controls for the efect of proposed turbine

**2F.Q2** When using a matching method, what is the main threat to causal identification?

*Response:*

Because matching alone doesn't solve the selection on unobservant problem.
...
There are 236 unique precincts that are ever treated.
...
The parallel trends assumption for the comparison from 2003 - 2007 and 2007 - 2011
...
```{r}
model1_ff <- lm(perc_lib ~ proposed_turbine + precinct_id + year + operational_turbine,
                sample_40_precincts) 
...
The signal-to-noise ratio for the treatment estimate is less than 2-to-1 estimate because the standard error should be less than half of the distribution of the coefficient.
...
The standard Error (SE) changed the SE estimates increased from the original method. Likely because the sampled data was assumed to have normal distributed residuals; however, repeated measurements within places can introduce correlated errors.
...
It is necessary to include year and precint as fixed effects to account for differences in liberal vote share overtime as well as within precincts. The liberal voter share in precinct is not depend on time, but precinct could become more or less liberal based on emigrating or immigrating to and from different precincts that change demographics of the precinct as well as motivational and individual political preferences. Between-level variance in precinct to precinct should be accounted for time-invariant difference, factors not dependent on time, for changes in the liberal voter share. resulting in remaining differences within-level differences that are time sensitive. By adding `year` as a fixed effect to the model, time-invariant differences from year to year in liberal voter share can be better accounted for to prevent OVB in within-level variance.
